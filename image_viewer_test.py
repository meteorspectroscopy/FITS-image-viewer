...
def show_image_array(image, imscale):
    """
    displays image in GUI, from m_specfun, only needed for comparison with old method
    ...
    if len(image.shape) == 3:
        ...
        multichannel = False
    ima = tf.rescale(image, imscale, multichannel=multichannel)
    ima = np.clip(ima, 0.0, 1.0)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        ios.imsave('tmp.png', np.flipud(ima * 255).astype(np.uint8))
        # image_element.update('tmp.png')  # not needed here
    return
...
layout = [[sg.Button('+'), sg.Button('-'), sg.Button('<'), sg.Button('>'),
           sg.Frame('', [[sg.Button('Slide_show'), sg.Checkbox('old method', key='old')]], border_width=2),
...
window = sg.Window('Comparison Image Viewer', layout, margins=(0, 0), element_padding=(0, 0), use_default_focus=False,
                   finalize=True)
...
while True:
    ...
    if event == 'Slide_show':
        ...
        if values['old']:  # previous method, saving images to 'tmp.png'
            for f in fnames:
                file_to_display = os.path.join(folder, f)
                if file_to_display.lower().endswith('.fit'):  # check for fits-files
                    im, hd = get_fits_image(file_to_display)
                    scale = min(G_SIZE[0] / im.shape[1], G_SIZE[1] / im.shape[0])
                    im = np.clip(im, 0.0, 1.0)
                    img = np.uint8(255 * im)  # convert to PIL format
                else:
                    img = np.flipud(img_as_float(ios.imread(file_to_display)))
                    scale = min(G_SIZE[0] / img.shape[1], G_SIZE[1] / img.shape[0])
                show_image_array(img, scale)
                idg = graph.DrawImage(filename='tmp.png',
                                      location=((g00[0] - G_SIZE[0]) // 2, (g00[1] + G_SIZE[1]) // 2))
                window['text'].Update(text_string + ' File: ' + f)
                window.refresh()
                graph.delete_figure(idg)
        else:  # new method, saving image to byte-array
            for f in fnames:
                file_to_display = os.path.join(folder, f)
